    ...
    <script>
        ...
        function membership(thisElement) {
            if ($('#instagramLink').text() !== 'خالی') {
                if ($('#instaProfileImg').attr('src').split('?')[0] != asset + 'img/SellerMajorProfileImage/Default/icon.jpg') {
        ...
        function deleteInstaPic() {
            $.confirm({
                title: 'حذف تصویر حساب اینستاگرام',
                content: 'آیا اطمینان دارید؟',
                buttons: {
                    تایید: function () {
                        window.location = '/Seller-Major-Remove-InstaProfileImg'
                    },
                    انصراف: function () {
                    },
                }
        ...
        let imgRowID = [], imgRowIdTemp, lastScrollTop = 0, lastScrollTopModal = 0, postNumber = 0, completeLoad = true,
            touchtime = 0, postDeleteItems = [], imgWorking = 0, asset = $('#asset').text(), imgNumber = 0, file_type,editPostStatus=false,
            dragging = false, imgAjax,
        ...
        document.addEventListener('click', function (event) {
            ...
            if ($('#postModal').length > 0 && $('#captionContainer').length>0) {
        ...
        function createDetailPost(posts, key, postHowDay, commentCount, userProfile) {
            ...
            profileImg = userProfile['Pic'] == 'img/SellerMajorProfileImage/Default/icon.jpg' ? 'img/SellerMajorProfileImage/Default/icon.jpg' : userProfile['Pic'] + '/profileImg.jpg';
        ...
        function sendComment(postID, textElement) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    profileImg = data[0]['Pic'] == 'img/SellerMajorProfileImage/Default/icon.jpg' ? 'img/SellerMajorProfileImage/Default/icon.jpg' : data[0]['Pic'] + '/profileImg.jpg';
                    $(tempCopy + ' .userInfo span').text(data[0]['name']);
                    $(tempCopy + ' .userInfo img').attr('src', asset+profileImg);
        ...
        function deletePost(postID, key) {
            ...
            $.ajax({
                type: 'GET',
                url: '/Seller-Major-Delete-Check/' + $('#sellerMajorID').text()+'/'+postID,
                success: function (data) {
                    console.log(data)
                    if (data) {
                        $.confirm({
                        ...
                        icon.addClass('icon-trash');
                        icon.removeClass('fa fa-spinner fa-spin');
                        alert('لطفا قبل از حذف آخرین پست خود ابتدا از طریق منوی کانون انعکاس (آیکون اینستاگرام) شرکت در کانون انعکاس را لغو و سپس اقدام به حذف آخرین پست نمایید.')
                    }
                }
            })
        }
        ...
        $(document).ready(function () {
            ...
            $('#Ad_overlay').modal('show');

            setTimeout(function () {
                $('#Ad_overlay').modal('hide');
            }, 7000);
            ...
            if (window.location.pathname === '/Seller-Major-Panel') {
                //auto resize all textarea height
                    ...
                    reader.onloadend = function () {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                $('#instaPicDelBtn').removeClass('d-none');
        ...
        function insertImg(key) {
            if($('#loadingImg'+key).hasClass('d-none')){
        ...
        $(document).ready(function () {
                        ...
                        imgAjax=$.ajax({
                            ...
                            success: function () {
                                ...
                                setTimeout(function () {
                                    ...
                                    $('#imgPreview' + key).show();
                                    $('#stopImgLoad' + key).hide();
                                },100)
                                $(imgWorking + ' .previewBox #imgPreview'+key).attr('src', reader.result);
                            }
                        });
                    };
                });
            ...
            if ($('.necessary').length > 0 && $('#captionContainer').hasClass('d-none')) {
        ...
        function resetImg(key){
            ...
            $('#addImgBtn-'+key+' h3').removeClass('d-none');
            $('#imgPreview' + key).hide();
            $('#stopImgLoad' + key).hide();
            imgAjax.abort();
        }
        ...
        function categoryUpdate() {
            $('#categoryUpdateIcon').addClass('d-none');
            $('#waitingCategoryUpdate').removeClass('d-none');
            $.ajax({
                type: 'GET',
                async: false,
                url: '/Seller-Major-CategoryUpdate/' + $('#hintCategory').val() + '/' + $('#category').val() + '/' + $('#catCode').val(),
                success: function (data) {
                    $('#modalCategory').modal('toggle');
                    $('#categoryLink').text(data);
                    console.log(data);
                }
            }).done(function () {
                $('#categoryUpdateIcon').removeClass('d-none');
                $('#waitingCategoryUpdate').addClass('d-none');
            });
        }
        ...
        function setFinalPrice(discount) {
            discount.val(fixNumbers(discount.val()).toString());
            if (discount.val() !== '') {
                if ($('#price').val() !== '') {
        ...
        function addComa(thisElement) {
            if ($('#price').val() !== '') {
                let price=fixNumbers(thisElement.val()).toString();
